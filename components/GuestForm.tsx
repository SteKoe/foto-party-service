...
export function GuestForm({ invitationOption, invitation }: AttendeeForm) {
  ...
  const guestChoices = invitation.Guest.GuestChoice?.reduce(
    (acc: any, curr: any) => {
      const rsvpOption = invitationOption.find(
        (rev) => rev.option_id === curr.option_id,
      )!;
        ...
        >
          {formState.saving
            ...
            : formState.success
              ? "Gespeichert!"
              : formState.error
                ? "Das hat nicht geklappt, versuche es nochmal!"
                : "Speichern"}
        </Button>
      </div>
    </form>
